...
class ErrorsTest(FaunaTestCase):
    ...
    def test_json_serialization(self):
        ...
        self.assertRaisesRegexCompat(UnexpectedError, msg,
                                     lambda: self.client.query(TestObj()))
    ...
    def test_unauthorized(self):
        ...
        self._assert_http_error(lambda: client.query(get(self.db_ref)),
    ...
    def test_error_cause(self):
        ...
        client.query(
            query.create_function({
                ...
                "my_abort_func",
                "body":
                query.query(query.lambda_([],
                                          query.abort("custom error here"))),
            }))

        # Make sure we can get that cusom message back
        exception = self.assert_raises(
            BadRequest, lambda: client.query(query.call("my_abort_func")))
        self._assert_error(exception,
                           "call error", [],
                           cause=[
                               ErrorData(code="transaction aborted",
                                         description="custom error here",
                                         position=["expr"],
                                         failures=None)
                           ])
    ...
    def test_invalid_expression(self):
        self._assert_query_error(_Expr({"foo": "bar"}), BadRequest,
                                 "invalid expression")
    ...
    def test_invalid_argument(self):
        self._assert_query_error(add([1, "two"]), BadRequest,
                                 "invalid argument", ["add", 1])
    ...
    def test_instance_not_found(self):
        ...
        self._assert_query_error(get(ref(collection("foofaws"), "123")),
                                 NotFound, "instance not found")

    def test_value_not_found(self):
        self._assert_query_error(query.select("a", {}), NotFound,
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(err),
            "ErrorData(code='code', description='desc', position=None, failures=None, cause=None)"
        )
