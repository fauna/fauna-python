...
class QueryTest(FaunaTestCase):
    ...
    def test_lambda_query(self):
        ...
        self.assertRaises(
            ValueError, lambda: self._q(
                query.map_(query.lambda_query(invalid_lambda), [])))
    ...
    def test_call_function(self):
        self._q(
            query.create_function({
                ...
                "concat_with_slash",
                "body":
                query.query(lambda a, b: query.concat([a, b], "/"))
            }))

        self.assertEqual(
            self._q(query.call(query.function("concat_with_slash"), "a", "b")),
            "a/b")
    ...
    def test_map(self):
        ...
        ns = query.map_(lambda a: query.select(["data", "n"], query.get(a)),
                        page)
    ...
    def test_filter(self):
        evens = query.filter_(lambda a: query.equals(query.modulo(a, 2), 0),
                              [1, 2, 3, 4])
        ...
        refs_with_m = query.filter_(
            lambda a: query.contains_path(["data", "m"], query.get(a)), page)
    ...
    def test_prepend(self):
        self.assertEqual(self._q(query.prepend([1, 2, 3], [4, 5, 6])),
                         [1, 2, 3, 4, 5, 6])
    ...
    def test_key_from_secret(self):
        db_ref = self.admin_client.query(
            query.create_database(
                {"name": "database_for_key_from_secret_test"}))["ref"]
        ...
        self.assertEqual(
            self.admin_client.query(query.key_from_secret(
    ...
    def test_move_database(self):
        ...
        self.admin_client.query(
            query.move_database(query.database("mvdb_src"),
                                query.database("mvdb_dst")))
        self.assertEqual(
            self.admin_client.query(query.exists(query.database("mvdb_src"))),
            False)
        self.assertEqual(
            self.admin_client.query(
                query.exists(
    ...
    def test_reduce(self):
        ...
        q1 = query.reduce(
            query.lambda_query(lambda accum, value: query.add(
                query.var("accum"), query.var("value"))), 0, data1)
        q2 = query.reduce(
            query.lambda_query(lambda accum, value: query.concat(
    ...
    def test_reverse(self):
        ...
        for i in range(100):
            self._q(
                query.create(query.collection("reverse_collection"),
                             {"data": {
                                 "val": i
                             }}))
        assertPaginate = lambda q, expected, size=None: self.assertEqual(
            self._q(query.select("data", query.paginate(q, size))), expected)

        assertPaginate(query.reverse(query.match(index)), [99, 98, 97, 96, 95],
                       5)
        assertPaginate(query.reverse(query.reverse(query.match(index))), list,
                       11)

        q1 = query.select(["data", 0],
                          query.reverse(
    ...
    def test_login_logout(self):
        ...
        self.assertEqual(
            instance_client.query(
                query.select(
                    "ref",
                    query.get(Ref("self", Ref("widgets",
                                              Native.COLLECTIONS))))),
            instance_ref)
    ...
    def test_identify(self):
        ...
        self.assertTrue(
            self.client.query(query.identify(instance_ref, "sekrit")))
    ...
    def test_casefold(self):
        ...
        self.assertEqual(self._q(query.casefold(u'\u1E9B\u0323', "NFKD")),
                         u'\u0073\u0323\u0307')
        self.assertEqual(self._q(query.casefold(u'\u1E9B\u0323', "NFKC")),
                         u'\u1E69')

        self.assertEqual(self._q(query.casefold(u'\u212B', "NFKCCaseFold")),
                         u'\u00E5')
    ...
    def test_contains_str_regex(self):
        ...
        self.assertEqual(
            self._q(query.contains_str_regex("faunadb", "/^\\d*\\.\\d+$/")),
            False)

    def test_regex_escape(self):
        self.assertEqual(self._q(query.regex_escape("f(\\w+)a")),
                         '\\Qf(\\w+)a\\E')
    ...
    def test_findstrregex(self):
        ...
        self.assertEqual(
            self._q(query.find_str_regex("one fish two Fish", "[fF]ish")),
            [{
                'start': 4,
                'end': 7,
                'data': 'fish'
    ...
    def test_replacestr(self):
        self.assertEqual(self._q(query.replace_str("ABCDE", "AB", "AA")),
                         "AACDE")
        self.assertEqual(
            self._q(query.replace_str("One Fish Two Fish", "Fish", "Cat")),
            "One Cat Two Cat")

    def test_replacestrregex(self):
        self.assertEqual(self._q(query.replace_str_regex("ABCDE", "AB", "AA")),
                         "AACDE")
        self.assertEqual(
            self._q(
                query.replace_str_regex("One Fish Two fish", "[Ff]ish",
                                        "Cat")), "One Cat Two Cat")
    ...
    def test_titlecase(self):
        self.assertEqual(self._q(query.titlecase("one fISH tWo FISH")),
                         "One Fish Two Fish")
    ...
    def test_select(self):
        ...
        self.assertEqual(self._q(query.select(["a", "c"], obj, 'default')),
                         'default')
    ...
    def test_round(self):
        ...
        self.assertEqual(self._q(query.round(123.456, 1)), 123.5)
    ...
    def test_to_object(self):
        self.assertEqual(self._q(query.to_object([['x', 0], ['y', 1]])), {
            'x': 0,
            'y': 1
        })
    ...
    def test_to_seconds(self):
        expected = (0)
        self.assertEqual(self._q(query.to_seconds(query.epoch(0, "second"))),
                         expected)

    def test_to_millis(self):
        expected = 1552733214259000
        self.assertEqual(
            self._q(query.to_millis(query.epoch(1552733214259, "second"))),
            expected)

    def test_to_micros(self):
        expected = 1552733214259000000
        self.assertEqual(
            self._q(query.to_micros(query.epoch(1552733214259, "second"))),
            expected)
    ...
    def test_hour(self):
        expected = 0
        self.assertEqual(self._q(query.hour(query.epoch(0, "second"))),
                         expected)
    ...
    def test_typecheckfns(self):
        ...
        self.admin_client.query(
            query.create_index({
                "name": "typecheck_index",
                "source": coll,
                "active": True
            }))
        doc = self.admin_client.query(
            query.create(coll, {
                "data": {},
                "credentials": {
                    "password": "spark2020"
        ...
        self.admin_client.query(query.create_database({"name":
                                                       "typecheck_db"}))
        function = self._q(
            query.create_function({
                "name":
                "typecheck_fn",
                "body":
                query.query(query.lambda_("x", query.now()))
            }))
        ...
        credentials = self._q(
            query.select(['data', 0], query.paginate(query.credentials())))
        role = self.admin_client.query(
            query.create_role({
                "name": "typecheck_role",
                "membership": [],
                "privileges": []
        ...
        for p in pairs:
            d = dict()
            d[p[0]] = query.count(
                query.filter_(query.lambda_("v", p[1](query.var("v"))),
                              query.var("vals")))
            q.append(d)
    ...
    def test_nested_keys(self):
        ...
        server_key_ref = Ref(server_key["ref"].id(),
                             cls=Ref("keys",
                                     db=Ref("db-for-keys",
                                            cls=Native.DATABASES)))
        admin_key_ref = Ref(admin_key["ref"].id(),
                            cls=Ref("keys",
                                    db=Ref("db-for-keys",
                                           cls=Native.DATABASES)))
