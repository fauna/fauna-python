...
class Ref(_Expr):
  ...
  """
    ...
    def __str__(self):
        col = ", collection=%s" % self.value[
            "collection"] if "collection" in self.value else ""
        db = ", database=%s" % self.value[
            "database"] if "database" in self.value else ""
        return "Ref(id=%s%s%s)" % (self.value["id"], col, db)

    def __repr__(self):
        col = ", collection=%s" % self.value[
            "collection"] if "collection" in self.value else ""
        db = ", database=%r" % self.value[
            "database"] if "database" in self.value else ""
        return "Ref(id=%s%s%s)" % (self.value["id"], col, db)
